...
def preprocess_qr(entries):
    ...
    for entry in entries:
        ...
        for item in resource["item"]:
            ...
            entry_template["resource"]["id"] = hash(
                json.dumps(
                    entry_template, default=str, ensure_ascii=True, sort_keys=True
                )
            )
...
def tabulate_qr(bundle):
    schema_path = (
        Path(__file__).parent / "DefaultQuestionnaireResponseSchema.yaml"
    )  # Path to a schema config file.
    schema = load_schema(schema_path)
    entries = get_bundle_entries_of_type(bundle, QuestionnaireResponse)
    preprocessed_data = preprocess_qr(entries)
    tabulated_results = tabulate_data(
        preprocessed_data, schema, "QuestionnaireResponse"
    )
